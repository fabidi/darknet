...
layer make_normalization_layer(int batch, int w, int h, int c, int size, float alpha, float beta, float kappa)
{
    ...
    layer.forward2 = forward_normalization_layer2;
...
{
    forward_normalization_layer2(layer, net, 0);
}

void forward_normalization_layer2(const layer layer, network net, int debugmode)
{
    if (debugmode)
...
//    mul_cpu(w*h*c*layer.batch, net.input, 1, layer.output, 1);
    if (debugmode)
        printf("Exiting forward_normalization_layer\n");
}
